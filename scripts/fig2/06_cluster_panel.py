...
INPUT_FILE = DATA_DIR / "GCMS_leaf.csv"
...
with open(META_DIR/"Fig1F_leaf_bestK.txt","w") as f:
...
z_mat[["Cluster"]].to_csv(META_DIR/"Fig1F_leaf_cluster_assignments.csv")
...
fig,axes=plt.subplots(best_k,1,figsize=(6,3.0*best_k),sharey=False)
...
for k in range(best_k):
    ...
    ax.fill_between(x_dense,ymin_s,ymax_s,color="#D5EAD9",alpha=0.45)

    for _,row in sub.iterrows():

        ax.plot(x, row.values, color="#9BD7F3", alpha=0.7, linewidth=0.8)

    ax.plot(x_dense,mean_s,color="#7DC69B",linewidth=2.2)
...
for ext in ["png","svg","pdf","tif"]:
    ...
    fig.savefig(
        ...
        OUT_DIR/f"Fig1F_leaf_kmeans.{ext}",
...
print("[Done] Fig1F leaf completed.")
