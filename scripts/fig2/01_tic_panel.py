...
data_path = "data/GCMS_leaf_TIC.csv"

USE_FIXED_RT_RANGE = True

RT_MIN, RT_MAX = 6.3, 40
...
df_leaf = df[df["Organ"].str.lower() == "leaf"].copy()

print(f"   叶片数据行数: {len(df_leaf)}")

df_leaf["Cultivar"] = pd.Categorical(
...
df_leaf = df_leaf[df_leaf["Cultivar"].notna()].copy()

global_rt_min = df_leaf["RT"].min()

global_rt_max = df_leaf["RT"].max()
...
df_leaf["TIC_scaled"] = df_leaf["TIC"] / TIC_SCALE

print("   TIC 统一按 1e7 缩放，y 轴将标为 TIC (×10⁷)")

df_leaf = df_leaf.sort_values(["Cultivar", "Stage", "RT"])
...
for i, cultivar in enumerate(cultivar_order):
    ...
    sub_cult = df_leaf[df_leaf["Cultivar"] == cultivar]
...
base = "output/Fig1A_leaf_TIC_7x2"
