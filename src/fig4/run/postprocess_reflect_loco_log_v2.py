...
def reflect_loco_log_space(pred_w: pd.DataFrame, true_w: pd.DataFrame) -> pd.DataFrame:

    true_w = true_w.sort_index()

    pred_w = pred_w.reindex(true_w.index).reindex(columns=true_w.columns)

    cultivars = true_w.index.to_list()

    vocs = true_w.columns.to_list()

    n = len(cultivars)

    if n < 3:

        raise ValueError("Need at least 3 cultivars for LOCO reflection to be meaningful.")

    Tlog = np.log1p(true_w.to_numpy(float))

    Plog = np.log1p(pred_w.to_numpy(float))

    sum_Tlog = Tlog.sum(axis=0, keepdims=True)

    mu = (sum_Tlog - Tlog) / float(n - 1)
...
def main():
    ...
    group_cols = ["panel"] if "panel" in df.columns else [None]
    ...
    for panel_val, g in groups:
        ...
        pred_fix_w = reflect_loco_log_space(pred_w, true_w)
    ...
    out_path = Path(args.out) if args.out else p.with_name(p.stem + "_reflect_log_loco.parquet")
