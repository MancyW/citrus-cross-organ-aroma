...
def main():
    ...
    peel_t = df_cs[(df_cs[organ_col] == peel_label) & (df_cs[stage_col] == target_stage)].copy()

    cultivars = sorted(peel_t[cultivar_col].unique().tolist())

    Y_true = []

    for c in cultivars:

        row = peel_t[peel_t[cultivar_col] == c]
    ...
    long.insert(1, "cv_mode", "fit_all")

    ts = datetime.now().strftime("run_%Y%m%d_%H%M%S")

    outdir = Path(args.outdir) if args.outdir else Path("results/pred_vectors") / f"{ts}_{args.panel.replace('+','_')}_FITALL"
